...
import { ArrowRight, ChevronRight, CheckCircle2, Play, MousePointer2, ArrowUpRight } from 'lucide-react';
...
const SITE_CONTENT = {
    ...
    hero: {
        headline: "Where AI Helps",
        headlineHighlight: "Business Grow.",
        subtext: "I build deterministic AI architectures for lead generation, content automation, and operational scaling. Stop following the hype. Start building systems.",
        actions: [
            { label: "Voice Receptionist", id: "voice" },
            { label: "Lead Sourcing", id: "sdr" },
            { label: "CRM Architect", id: "crm" },
            { label: "ROI Analysis", id: "roi" }
        ]
    },
    manifesto: {
        ...
        <nav className="fixed top-0 left-0 w-full z-50 py-8 lg:py-10 px-6 flex justify-center pointer-events-none">
            ...
            >
                ...
                <div className="hidden md:flex items-center gap-8 text-base font-medium text-ivory/80">
                    <Link to="/#manifesto" onClick={(e) => { if(window.location.pathname === '/') { e.preventDefault(); document.getElementById('manifesto')?.scrollIntoView({behavior: 'smooth'}) } }} className="hover:text-champagne transition-colors transform hover:-translate-y-[1px]">The Reality</Link>
                    <Link to="/#features" onClick={(e) => { if(window.location.pathname === '/') { e.preventDefault(); document.getElementById('features')?.scrollIntoView({behavior: 'smooth'}) } }} className="hover:text-champagne transition-colors transform hover:-translate-y-[1px]">The Engine</Link>
                    <Link to="/lab" className="hover:text-champagne transition-colors transform hover:-translate-y-[1px]">The Lab</Link>
                    <Link to="/#protocol" onClick={(e) => { if(window.location.pathname === '/') { e.preventDefault(); document.getElementById('protocol')?.scrollIntoView({behavior: 'smooth'}) } }} className="hover:text-champagne transition-colors transform hover:-translate-y-[1px]">Protocol</Link>
                </div>
                <a
...
const Hero = () => {
    ...
    return (
        <section ref={heroRef} className="relative min-h-[100dvh] w-full flex flex-col items-center justify-center pt-32 pb-24 px-6 md:px-16 overflow-hidden bg-obsidian">
            {/* Minimalist Background Blobs */}
            <div className="hero-bg-blob absolute top-1/4 -left-1/4 w-[600px] h-[600px] bg-champagne/10 rounded-full blur-[150px] pointer-events-none"></div>
            <div className="hero-bg-blob absolute bottom-1/4 -right-1/4 w-[500px] h-[500px] bg-slate/10 rounded-full blur-[150px] pointer-events-none"></div>

            <div className="relative z-10 max-w-6xl mx-auto flex flex-col items-center text-center" ref={textRef}>
                <div className="font-mono text-champagne mb-12 text-xs md:text-sm tracking-[0.4em] uppercase py-2 px-6 border border-white/10 rounded-full bg-white/5 backdrop-blur-sm">
                    Protocol 0.1 Beta | Live Lab Ready
                </div>

                <h1 className="font-ui font-bold text-6xl md:text-8xl lg:text-[10rem] tracking-tighter leading-[0.85] mb-12 text-ivory">
                    {SITE_CONTENT.hero.headline}<br />
                    <span className="text-champagne italic font-drama">{SITE_CONTENT.hero.headlineHighlight}</span>
                </h1>

                <p className="font-ui text-lg md:text-2xl text-ivory/60 max-w-3xl mb-16 leading-relaxed text-balance">
                    {SITE_CONTENT.hero.subtext}
                </p>

                {/* The "Action Bank" - Inspired by wellfound Search/Category feel */}
                <div ref={actionsRef} className="flex flex-wrap justify-center gap-4 max-w-4xl">
                    {SITE_CONTENT.hero.actions.map((action) => (
                        <Link 
                            key={action.id}
                            to="/lab"
                            className="group relative flex items-center gap-3 bg-white/5 border border-white/10 px-8 py-5 rounded-2xl hover:bg-champagne hover:border-champagne transition-all duration-500 hover:-translate-y-1"
                        >
                            <span className="font-ui font-semibold text-ivory group-hover:text-obsidian transition-colors text-lg">
                                {action.label}
                            </span>
                            <ArrowUpRight className="text-champagne group-hover:text-obsidian transition-colors" size={20} />
                        </Link>
                    ))}
                    <a 
                        href={SITE_CONTENT.links.calendar}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-3 bg-ivory text-obsidian px-8 py-5 rounded-2xl hover:bg-champagne transition-all duration-500 hover:-translate-y-1 font-ui font-bold text-lg"
                    >
                        Book Call
                    </a>
                </div>
            ...
            {/* Visual Continuity - Scrolling indicator */}
            <div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex flex-col items-center gap-4 opacity-20">
                <span className="font-mono text-[10px] uppercase tracking-widest text-ivory">Engine Status: Online</span>
            ...
            <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
                {SITE_CONTENT.agenticLab.map((project, i) => (
                    <div key={i} className="group relative bg-[#121218] rounded-[4rem] p-12 lg:p-16 border border-ivory/5 hover:border-champagne/30 transition-all duration-500 overflow-hidden cursor-pointer">
                        ...
                        <Link to="/lab" className="mt-14 flex items-center gap-4 text-champagne font-mono text-sm opacity-0 group-hover:opacity-100 transition-opacity transform translate-y-4 group-hover:translate-y-0 duration-500 relative z-10">
                            <span>Check The Lab</span>
                            <ArrowRight size={18} />
                        </Link>
                    </div>
                ))}
...
const HomeLayout = () => {
    ...
    // Quick hash scrolling fix when returning to home layout
    useEffect(() => {
        if (location.hash) {
